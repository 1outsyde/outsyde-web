...
export async function sendNotifySignupAlert(p: { email: string; vendorName?: string }) {
  const context = p.vendorName ? `${p.vendorName} coming-soon page.` : "Coming Soon page.";
  const html = `<!DOCTYPE html><html><body style="margin:0;padding:0;background-color:#0a0a0a;font-family:Helvetica,Arial,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background-color:#0a0a0a;padding:40px 20px;"><tr><td align="center">
  <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
    <tr><td align="center" style="padding:0 0 32px 0;">
    ...
    <tr><td style="background-color:#141414;padding:24px;border-radius:0 0 4px 4px;">
      ...
      <p style="color:#666;margin:16px 0 0 0;font-size:12px;">Captured from the ${context}</p>
    </td></tr>
    <tr><td align="center" style="padding:24px 0 0 0;"><p style="color:#444;font-size:11px;margin:0;">© ${new Date().getFullYear()} Go Outsyde. All rights reserved.</p></td></tr>
  </table></td></tr></table></body></html>`;

  const subject = p.vendorName
    ? `🔔 New waitlist signup — ${p.vendorName}: ${p.email}`
    : `🔔 New notify-list signup: ${p.email}`;
...
export async function sendNotifySignupConfirmation(p: { email: string; vendorName?: string }) {
  const headline = p.vendorName ? `${p.vendorName} is coming to OutsYde.` : "You're on the list";
  const body1 = p.vendorName
    ? `You're on the early access list for ${p.vendorName}. We'll reach out the moment they open on OutsYde — you'll be among the first to book and discover what they offer.`
    : "Thanks for signing up. You'll be among the first to know the moment this part of Outsyde goes live.";
  const subject = p.vendorName
    ? `🎉 You're on the list — ${p.vendorName} is coming to OutsYde`
    : `🎉 You're on the list — Go Outsyde`;
  const html = `<!DOCTYPE html><html><body style="margin:0;padding:0;background-color:#0a0a0a;font-family:Helvetica,Arial,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background-color:#0a0a0a;padding:40px 20px;"><tr><td align="center">
  <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
    <tr><td align="center" style="padding:0 0 32px 0;">
      <h1 style="color:#E8B930;font-size:28px;letter-spacing:4px;margin:0;text-transform:uppercase;">GO OUTSYDE</h1>
      <p style="color:#666;font-size:11px;letter-spacing:2px;margin:4px 0 0 0;text-transform:uppercase;">Like A Lavished Local</p></td></tr>
    <tr><td style="background-color:#E8B930;padding:16px 24px;border-radius:4px 4px 0 0;"><h2 style="color:#0a0a0a;margin:0;font-size:18px;">🎉 ${headline}</h2></td></tr>
    <tr><td style="background-color:#141414;padding:24px;border-radius:0 0 4px 4px;">
      <p style="color:#ffffff;font-size:15px;line-height:1.7;margin:0 0 16px 0;">${body1}</p>
