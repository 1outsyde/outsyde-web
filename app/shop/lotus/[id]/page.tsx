...
import { getLotusProducts, getProductGroup, getProductFormat } from '@/lib/outsyde-products';
...
.pd-media .ptype-badge{position:absolute;top:1rem;left:1rem;z-index:2;font-size:.58rem;letter-spacing:.18em;text-transform:uppercase;padding:.4rem .75rem;background:#fff;color:var(--lhb-text-mid);font-weight:500;}
...
.related{max-width:1100px;margin:0 auto;padding:0 1.5rem 5rem;}
.related-hdr{font-family:'Cormorant Garamond',serif;font-size:1.7rem;font-weight:500;color:var(--lhb-brown-dark);margin-bottom:.4rem;}
.related-sub{font-size:.78rem;color:var(--lhb-text-muted);margin-bottom:1.75rem;letter-spacing:.04em;}
...
.loading{max-width:600px;margin:0 auto;text-align:center;padding:6rem 1.5rem;color:var(--lhb-text-muted);font-family:'Jost',sans-serif;}
...
export default function LotusProductPage() {
  ...
  const [product, setProduct] = useState<VendorProduct | null | undefined>(undefined);
  const [related, setRelated] = useState<VendorProduct[]>([]);
  const [cart, setCart] = useState<CartItem[]>([]);
  const [qty, setQty] = useState(1);
  const [added, setAdded] = useState(false);

  useEffect(() => {
    getLotusProducts().then((products) => {
      const found = products.find((p) => p.id === id) ?? null;
      setProduct(found);
      if (found) {
        const group = getProductGroup(found);
        setRelated(products.filter((p) => p.id !== id && getProductGroup(p) === group));
      }
    });
  }, [id]);
  ...
  const fmt = getProductFormat(product);
  const fmtLabel = fmt === 'tea' ? 'Tea Box' : fmt === 'herbs' ? 'Loose Herbs' : null;
    ...
    <div className="lhb-pd">
      ...
      <nav className="outsyde-nav">
        ...
        <div className="nav-right">
          <a href="/shop/lotus" className="nav-back">← Lotus House Blends</a>
      ...
      <div className="crumb">
        <a href="/shop/lotus">Lotus House Blends</a> &nbsp;/&nbsp; {group} &nbsp;/&nbsp; {product.name}
      </div>

      <div className="pd-wrap">
        <div className="pd-media">
          {fmtLabel && <span className="ptype-badge">{fmtLabel}</span>}
          {product.imageUrl ? (
            // eslint-disable-next-line @next/next/no-img-element
            <img src={product.imageUrl} alt={product.name} />
          ) : (
            <div style={{ width: '100%', aspectRatio: '1/1', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--lhb-text-muted)', fontSize: '.9rem' }}>
              No image
            </div>
          )}
        </div>
        ...
        <div className="pd-info">
          <div className="pd-eyebrow">{group}</div>
          <h1 className="pd-title">{product.name}</h1>
          {fmtLabel && <div className="pd-format">{fmtLabel}</div>}

          <div className="pd-price-row">
            <span className="pd-price">${(product.price / 100).toFixed(2)}</span>
          </div>
          ...
          <div className="pd-buy">
            ...
            <button className="add-btn" onClick={handleAdd}>
              Add to Cart — ${((product.price / 100) * qty).toFixed(2)}
            </button>
          </div>
      ...
      {related.length > 0 && (
        <section className="related">
          <h2 className="related-hdr">More from {group}</h2>
          <p className="related-sub">Other products in this blend.</p>
          <div className="related-grid">
            {related.map((r) => (
              <a className="rcard" href={`/shop/lotus/${r.id}`} key={r.id}>
                {r.imageUrl ? (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img src={r.imageUrl} alt={r.name} />
                ) : (
                  <div style={{ width: 72, height: 72, background: '#e8e0d0', flexShrink: 0 }} />
                )}
                <div>
                  <div className="rname">{r.name}</div>
                  <div className="rmeta">${(r.price / 100).toFixed(2)}</div>
                </div>
              </a>
            ))}
          </div>
